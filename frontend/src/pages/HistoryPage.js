    ...
    <div className="history-page">
      <div className="container">
        <div className="page-header text-center mb-4">
          ...
          <p className="page-subtitle">View all previous dowry predictions</p>
        </div>
        ...
        <div className="history-container">
          ...
          <div className="history-filters mb-4">
            <div className="filter-buttons">
              ...
              <button
                className={`btn ${filter === 'male' ? 'btn-primary' : 'btn-outline'} ml-2`}
                onClick={() => setFilter('male')}
              >
                Male ({history.filter(h => h.gender === 'Male').length})
              </button>
              <button
                className={`btn ${filter === 'female' ? 'btn-primary' : 'btn-outline'} ml-2`}
                onClick={() => setFilter('female')}
              >
                Female ({history.filter(h => h.gender === 'Female').length})
              </button>
              <button
                className={`btn ${filter === 'high' ? 'btn-success' : 'btn-outline'} ml-2`}
                onClick={() => setFilter('high')}
              >
                High Dowry ({history.filter(h => h.prediction.predicted_category === 'High Dowry').length})
              </button>
              <button
                className={`btn ${filter === 'low' ? 'btn-warning' : 'btn-outline'} ml-2`}
          ...
          {filteredHistory.length === 0 ? (
            <div className="card p-4 text-center">
              ...
              <h4 className="mt-2">No History Found</h4>
          ...
          {/* Summary */}
          {filteredHistory.length > 0 && (
            <div className="history-summary mt-4">
